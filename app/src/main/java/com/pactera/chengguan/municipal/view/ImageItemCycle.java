...
import com.pactera.chengguan.municipal.adapter.CycleViewPager;
 ...
 * </pre>
 */
public class ImageItemCycle extends LinearLayout {
     ...
     * @param context
     */
    public ImageItemCycle(Context context) {
        super(context);
    }
     ...
     * @param attrs
     */
    public ImageItemCycle(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        mScale = context.getResources().getDisplayMetrics().density;
        LayoutInflater.from(context).inflate(R.layout.approval_banner_content, this);
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_UP:
                        // 开始图片滚动
                        startImageTimerTask(move);
                        break;
                    default:
                        // 停止图片滚动
                        stopImageTimerTask(move);
                        break;
                }
                return false;
            }
     ...
     */
    public void startImageCycle(boolean move) {
        startImageTimerTask(move);
    }
     ...
     * 暂停轮播——用于节省资源
     */
    public void pushImageCycle(boolean move) {
        stopImageTimerTask(move);
    }
     ...
     * 开始图片滚动任务
     */
    private void startImageTimerTask(boolean move) {
        if (move) {
            stopImageTimerTask(move);
            // 图片每3秒滚动一次
     ...
     * 停止图片滚动任务
     */
    private void stopImageTimerTask(boolean move) {
        if (move) {
    ...
    private final class GuidePageChangeListener implements OnPageChangeListener {
        ...
        public void onPageScrollStateChanged(int state) {
            if (state == ViewPager.SCROLL_STATE_IDLE)
                startImageTimerTask(move); // 开始下次计时
        }
    ...
    private class ImageCycleAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            if (mAdList.size() <= 0 || mAdList.size() <= position) {
                return null;
            }
