...
import com.pactera.chengguan.municipal.adapter.CaseListAdapter;
...
import com.pactera.chengguan.municipal.bean.municipal.CaseListBean;
...
public class CaseListActivity extends BaseActivity implements OnRefreshListener, OnLoadMoreListener
        , AdapterView.OnItemClickListener,AdapterView.OnItemLongClickListener, com.pactera.chengguan.municipal.config.RequestListener {
    private static final String[] tab_one = {"待办", "处理中", "办结"};
    private static final String[] tab_two = {"全部", "月度", "季度", "年度", "日常"};
    private static final String[] tab_three = {"全部", "一月", "二月", "三月", "四月", "五月", "六月"
            , "七月", "八月", "九月", "十月", "十一月", "十二月"};
    private static final String[] tab_four = {"时间排序", "超限排序"};
    private String headers[] = {"待办", "全部", "全部", "时间排序"};
...
//        swipeTarget.setOnItemLongClickListener(this);
    ...
    @Override
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        com.pactera.chengguan.municipal.model.municipal.CaseInfo caseInfo = caseInfoList.get(position);
        Intent intent;
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
       ArrayList<String> datas=new ArrayList<String>();
        ...
        datas.add("测试");
        datas.add("测试");
        ListDialog listDialog=new ListDialog(mContext, R.style.dialog_dimenable, new ListDialog.OnClickDialogListener() {
            @Override
            public void onClickItem(AdapterView<?> parent, View view, int position, long id) {

            }
        });
                listDialog.show(new ListDialogAdapter(mContext, datas));
        return true;
    }
    ...
    private void requestCaseListData(int requestStatus){
        ...
        Integer category = null, month = null;
        if(selectTabTwoIndex > 0){
            category = selectTabTwoIndex;
        }
        if(selectTabThreeIndex > 0){
            month = selectTabThreeIndex;
        }
        com.pactera.chengguan.municipal.util.MunicipalRequest.requestCaseList(this, this, selectTabOneIndex + 1, category, month
    ...
    public void success(String reqUrl, Object result) {
        ...
        com.pactera.chengguan.municipal.bean.municipal.CaseListBean caseListBean = (com.pactera.chengguan.municipal.bean.municipal.CaseListBean)result;
    ...
    private BaseHandler caseListHandler = new BaseHandler() {
        ...
        public void doSuccess(BaseBean baseBean, String message) {
            ...
            List<com.pactera.chengguan.municipal.model.municipal.CaseInfo> list = caseListBean.transformCaseInfo();
